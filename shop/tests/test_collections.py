...
@pytest.fixture
def collection():
    return Collection.objects.create(title='Fake Product')

@pytest.mark.django_db
class TestCreateCollection():
    def test_if_user_is_anonymous_return_401(self,api_client,collection):
        response = api_client.post('/shop/collections/',{'title':'product'})
    ...
    def test_if_user_is_staff_and_data_is_valid_return_201(self,api_client,authenticate,collection):
        ...
        response = api_client.post('/shop/collections/',{'title':'Fake Product 2'})
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data['title'] == 'Fake Product 2'
...
class TestRetriveCollection():
    ...
    def test_if_user_is_staff_return_200(self,api_client,authenticate,collection):
        ...
        response = api_client.get('/shop/collections/')
        assert response.status_code == status.HTTP_200_OK
        assert response.json() and response.json()[0]['title'] == 'Fake Product'
...
class TestUpdateCollection():
    def test_if_user_is_anonymous_return_401(self,api_client,collection):
        response = api_client.patch('/shop/collections/1/',{'title':'New Fake Product'})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_if_user_is_staff_and_data_is_valid_return_200(self,api_client,authenticate,collection):
        ...
        response = api_client.patch('/shop/collections/1/',{'title':'New Fake Product'})
        assert response.status_code == status.HTTP_200_OK
        assert response.json()['title'] == 'New Fake Product'
